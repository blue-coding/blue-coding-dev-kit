...
├── README.md
├── install-all.sh                          # Interactive unified installer (theme + terminal)
├── setup-claude.sh                         # One-time Claude Code setup (hooks + skill)
│
├── .claude/
│   ├── hooks/
│   │   ├── bc-env-guard.sh                 # Blocks Claude from reading .env files
│   │   └── bc-push-auth.sh                 # Requires explicit confirmation for git push/commit
│   ├── skills/
│   │   └── blue-coding-design-system/      # Brand design system skill for Claude Code
│   └── settings.local.json                 # Generated by setup-claude.sh (not committed)
│
...
│   ├── blue-coding-dark.vsix               # Pre-packaged extension (drag-and-drop install)
│   └── blue-coding-dark/
...
```bash
# Install VS Code theme + terminal (interactive)
bash ~/blue-coding-dev-kit/install-all.sh

# Configure Claude Code hooks and design system skill (run once after cloning)
bash ~/blue-coding-dev-kit/setup-claude.sh
```
...
#### Option B — VSIX package (drag-and-drop)

1. Open VS Code
2. `Ctrl+Shift+P` → `Extensions: Install from VSIX…`
3. Select `vscode-theme/blue-coding-dark.vsix`
4. Restart VS Code

#### Option C — Manual (source folder)
...
Running `setup-claude.sh` once after cloning configures two things in your Claude Code installation:

### Security hooks

Two `PreToolUse` hooks are wired into `.claude/settings.local.json` with absolute paths for your machine:

| Hook | Trigger | What it does |
| --- | --- | --- |
| `bc-env-guard.sh` | Any `Read` or `Bash` call | Blocks Claude from reading `.env` files. Also warns if the file is not in `.gitignore`. |
| `bc-push-auth.sh` | Any `Bash` call | Intercepts `git push` and `git commit` and requires explicit user approval before proceeding. |

### Design system skill

Installs the `blue-coding-design-system` skill to `~/.claude/skills/` so it is available globally in every Claude Code session — not just inside this repo. The skill gives Claude access to the full brand guidelines, color tokens, typography, logos, and icon set for generating on-brand UI assets and documents.
...
Re-run any time you move the repo to a new path — the script regenerates `settings.local.json` with the correct absolute paths.
...
- `jq` and `rsync` available in PATH (used by `setup-claude.sh`)
